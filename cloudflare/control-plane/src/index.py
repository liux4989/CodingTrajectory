    ...
    async def fetch(self, request: Any) -> Response:
        ...
        try:
            ...
            token_match = re.fullmatch(
                r"Bearer ([A-Za-z0-9_-]{32,256})", authorization or ""
            )
            require_that(token_match, "authentication_required", 401)
            token = token_match.group(1)
            try:
                registry = object_value(_json_loads(self.env.CT_PRINCIPALS))
            ...
            require_that(
                ...
                and all(
                    role in {"read", "collect", "owner"} for role in principal["roles"]
                ),
                "invalid_principal",
                503,
            )
