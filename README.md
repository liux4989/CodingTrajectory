...
- `SessionGraph` is a structural aggregate over canonical sessions. Its identity is the root session id, and it may expose graph-level structure such as membership, edges, and summary metadata. The CLI keeps this aggregate behind explicit `ct graph` commands; `ct session` is one thread.
