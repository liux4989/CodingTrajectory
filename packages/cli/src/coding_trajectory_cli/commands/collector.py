...
from coding_trajectory_cli._shared import GhFormatter, add_agent_vendor_flag, add_global_scope_flag
...
def _state_path(value: str | None) -> Path:
    return Path(value).expanduser() if value else Path("~/.coding-trajectory/control-plane/collector.sqlite3").expanduser()
...
def _remote_from_args(args: argparse.Namespace) -> SupabaseCollectorRemote:
    ...
    missing = [
        name
        for name, value in (("SUPABASE_URL", url), ("SUPABASE_ANON_KEY", api_key), ("COLLECTOR_ACCESS_TOKEN", access_token))
        if not value
    ]
    if missing:
...
def _identity_from_args(args: argparse.Namespace, state_path: Path) -> CollectorIdentity:
...
def _handle_scan(args: argparse.Namespace) -> dict[str, Any]:
    candidates = discover_source_candidates(
        current_dir=Path.cwd(), global_scope=args.global_scope, agent_vendor=args.agent_vendor
    )
    return {
        "discovered": len(candidates),
        "by_vendor": dict(sorted(Counter(item.vendor.value for item in candidates).items())),
...
def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    scan = commands.add_parser("scan", help="Count eligible local sources without reading content.", formatter_class=GhFormatter)
    add_global_scope_flag(scan)
    add_agent_vendor_flag(scan)
    scan.add_argument("--since-days", type=_positive_int, help="Include sources modified in the last N days.")
    scan.set_defaults(_plugin_handler=_handle_scan, _default_output="json")

    run = commands.add_parser("run", help="Queue and publish normalized canonical snapshots.", formatter_class=GhFormatter)
    add_global_scope_flag(run)
    add_agent_vendor_flag(run)
    run.add_argument("--since-days", type=_positive_int, help="Publish only sources modified in the last N days.")
    run.add_argument("--workspace-id", required=True, type=_uuid_arg)
    run.add_argument("--agent-id", required=True, type=_uuid_arg)
    ...
    status = commands.add_parser("status", help="Show local outbox count only.", formatter_class=GhFormatter)
