"""Shared Codex app-server transport owned by the application layer.

Moved out of the dashboard plugin so core services (for example the
estimation subsystem) can run bounded semantic inference turns without
importing another plugin. The dashboard plugin re-exports these names for
...
class CodexAppServerClient:
    ...
    def run_turn(
        ...
        *,
        ...
        service_name: str = "coding-trajectory",
...
class CodexAppServerManager:
    """Owns one app-server subprocess for a long-lived server lifetime.
    ...
    Callers that need concurrency must use a bounded pool of independent
    sessions instead.
    """
        ...
        self._send(
            {
                ...
                "params": {
                    "clientInfo": {
                        "name": "coding_trajectory",
                        "title": "CodingTrajectory",
                        "version": "0.1.0",
                    },
                    "capabilities": {
