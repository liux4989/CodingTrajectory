"""Trajectory indexes derived from canonical ingestion models."""
...
from coding_trajectory.ingestion.models import Event, Session, Step, Trajectory, TrajectoryEdge, Turn
...
class TrajectoryIndex:
    ...
    incoming_edge_by_target: dict[UUID, TrajectoryEdge]
    incoming_edge_type: dict[UUID, str | None]
    outgoing_edges_by_source_step: dict[UUID, list[TrajectoryEdge]]
    roots: list[UUID]


def build_trajectory_index(trajectory: Trajectory) -> TrajectoryIndex:
    sessions_by_id: dict[UUID, Session] = {}
    turns_by_id: dict[UUID, Turn] = {}
    steps_by_id: dict[UUID, Step] = {}
    events_by_id: dict[UUID, Event] = {}
    ...
    incoming_edge_by_target: dict[UUID, TrajectoryEdge] = {}
    outgoing_edges_by_source_step: dict[UUID, list[TrajectoryEdge]] = defaultdict(list)
    ...
    return TrajectoryIndex(
        session_ids_in_order=[session.session_id for session in trajectory.sessions],
...
def ordered_sessions(index: TrajectoryIndex) -> list[Session]:
...
def incoming_edge(index: TrajectoryIndex, session_id: UUID) -> TrajectoryEdge | None:
    return index.incoming_edge_by_target.get(session_id)


def events_for_step(index: TrajectoryIndex, step: Step) -> list[Event]:
    return [index.events_by_id[event_id] for event_id in step.event_ids if event_id in index.events_by_id]


def events_for_turn(index: TrajectoryIndex, turn: Turn) -> list[Event]:
    return [index.events_by_id[event_id] for event_id in turn.event_ids if event_id in index.events_by_id]


def event_for_turn_user_request(index: TrajectoryIndex, turn: Turn) -> Event | None:
