...
def serve_estimate(
    ...
    *,
    ...
    if method == "estimate.get":
        return {"forecast": ledger.find_forecast(params["prediction_id"])}
    if method == "estimate.list":
        return _list(params, ledger=ledger)
    if method == "estimate.calibration":
        return _calibration(params, ledger=ledger)
...
def run_forecast_for_turn(
    ...
    *,
    ...
    ledger.append_comparison(record["prediction_id"], comparison)
...
def _bind(
    ...
    *,
    ...
    bound_at = format_datetime(datetime.now(timezone.utc))
    receipt = {
        "bound_at": bound_at,
        "turn_id": str(turn_id),
        "root_session_id": str(candidate.root_session_id),
...
def _binding_window_check(
    ...
    if not (
        candidate.task_available_at <= issued_at < candidate.target_execution_started_at
    ):
        return {
            "state": "permanent_failed",
...
def _list(params: dict[str, Any], *, ledger: ForecastLedger) -> dict[str, Any]:
...
def _calibration(params: dict[str, Any], *, ledger: ForecastLedger) -> dict[str, Any]:
    filters = {
        key: params.get(key)
        for key in (
            "forecast_kind",
