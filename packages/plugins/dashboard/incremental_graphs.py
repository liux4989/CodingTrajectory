"""Canonical graph repair from transaction-persisted source messages.

This module is the bridge between :mod:`incremental_store` and the canonical
ingestion layer.  It deliberately never calls ``ingest_file`` or
``scan_header``: all JSONL records are supplied by the caller from the current
SQLite transaction.  The canonical adapters' protected record-to-model helpers
are used because they are presently the only implementation of the vendor
projection semantics.  Keeping those calls here, behind Pydantic boundaries,
makes that dependency explicit and easy to replace with a public core API.
...
from coding_trajectory.discovery import infer_project_identifier, stabilize_session
from coding_trajectory.ingestion import ClaudeCodeAdapter, CodexAdapter, PiAdapter
from coding_trajectory.ingestion.adapters.base import BaseAdapter
from coding_trajectory.ingestion.adapters.claude_code import _subagent_input
from coding_trajectory.ingestion.adapters.codex import _cut_inherited_records
...
try:
    from .incremental_store import SourceMessage, SourceSnapshot
except ImportError:  # pragma: no cover - direct plugin execution compatibility
    from incremental_store import SourceMessage, SourceSnapshot
    ...
    for position, raw in enumerate(sources):
        try:
            snapshot = (
                ...
                else SourceSnapshot.model_validate(raw)
            )
...
def _validated_message(path: str, raw: MessageInput) -> SourceMessage:
    message = (
        raw if isinstance(raw, SourceMessage) else SourceMessage.model_validate(raw)
    )
