"""Dashboard-owned models.dev pricing and context metadata."""
...
def estimate_cost(
    ...
    *,
    ...
    if rule is None:
        return None
    breakdown = _estimate_usage(TokenUsage.model_validate(usage or {}), rule, provider=provider)
...
def _refresh_models_dev_cache(*, now: datetime) -> ModelsDevCacheArtifact | None:
    request = Request(MODELS_DEV_SOURCE, headers={"User-Agent": "coding-trajectory-dashboard/1.0"})
    try:
...
def _should_refresh_live_pricing() -> bool:
    return os.environ.get("CT_DASHBOARD_DISABLE_LIVE_PRICING") not in {"1", "true", "TRUE"}
...
def _model_to_price_rule(
    *,
    ...
    return PriceRule(
        ...
        output_per_mtok_above_threshold=threshold_cost.output if threshold_cost else None,
