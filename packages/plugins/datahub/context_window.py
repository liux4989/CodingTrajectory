...
def build_projection(
    ...
    *,
    ...
    # the merged static/live model catalog) — the datahub reads it directly.
...
def _load_projection_inputs(
    ...
    if not isinstance(stats.get("root_session_id"), str):
        raise TypeError("ct api request returned invalid result: stats")
    if not isinstance(overview.get("root_session_id"), str):
        raise TypeError("ct api request returned invalid result: overview")
    if not isinstance(usage.get("session_id"), str) or not isinstance(
        usage.get("total_usage"), dict
    ):
        raise TypeError("ct api request returned invalid result: usage")
    if not isinstance(tool_usage.get("root_session_id"), str):
        raise TypeError("ct api request returned invalid result: tool_usage")
    return (
...
def _required_batch_result(
    ...
    if not isinstance(result, dict):
        raise TypeError(f"ct api request returned invalid result: {request_id}")
    return result
    ...
    *,
    ...
    for turn in turns:
        ...
        if confirmed is not None:
            ...
            model_to = None
        prev_model_key = model_key or prev_model_key
        # Core prices the observable boundary cache-hit loss; the datahub
...
def _projection_warnings(events: list[ContextEvent]) -> list[str]:
    ...
    warnings = [
        (
            "Turn usage is cumulative model accounting and is retained as a detail "
            "reference, not presented as context added by one timeline event."
        ),
    ]
