...
export function ComparisonTable({ rows }: { rows: ComparisonRow[] }) {
  return (
    <Card className="content-section min-w-0">
      ...
      <CardContent>
        {rows.length ? <Table>
          <TableHeader><TableRow><TableHead>Provider / model</TableHead><TableHead>Graphs</TableHead><TableHead>Tokens</TableHead><TableHead>Cache</TableHead><TableHead>Cost</TableHead><TableHead>Pricing</TableHead></TableRow></TableHeader>
          <TableBody>
            {rows.map((row) => (
...
export function SessionTable({ rows }: { rows: SessionRow[] }) {
  return (
    <Card className="content-section min-w-0">
      ...
      <CardContent>
        {rows.length ? <Table>
          <TableHeader><TableRow><TableHead>Session graph</TableHead><TableHead>Model</TableHead><TableHead>Tokens</TableHead><TableHead>Cost evidence</TableHead><TableHead>Active</TableHead><TableHead>Wait</TableHead><TableHead>Turns</TableHead></TableRow></TableHeader>
          <TableBody>
            {rows.map((row) => (
